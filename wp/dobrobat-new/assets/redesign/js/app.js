(() => {
    ...
    function disableScrollAndSwipes(scrollPosition) {
        ...
        elementsNodeList_header.style.top = `0px`;
        ...
        if (elementsNodeList_header) {
            ...
            links.forEach((link => {
                link.addEventListener("click", (function(event) {
                    ...
                    const headerHeight = elementsNodeList_header.offsetHeight;
                    if (burger && burger.classList.contains("active")) {
                        ...
                        setTimeout((() => {
                            ...
                            elementsNodeList_header.classList.remove("active");
                        }), 0);
                    }
                    window.scrollTo({
    ...
    function headerFixed() {
        if (elementsNodeList_header) {
            ...
            if (scrollY >= elementsNodeList_header.clientHeight) elementsNodeList_header.classList.add("fixed");
            ...
            const scrollHandler = throttle((() => {
                ...
                try {
                    if (!elementsNodeList_header.classList.contains("active")) if (scrollY >= elementsNodeList_header.clientHeight / 2.5) elementsNodeList_header.classList.add("fixed"); else elementsNodeList_header.classList.remove("fixed");
                    if (scrollY >= elementsNodeList_header.clientHeight + 60) {
                        if (currentScrollPosition > lastScrollPosition) elementsNodeList_header.classList.add("header-hidden"); else elementsNodeList_header.classList.remove("header-hidden");
    ...
    const burger = () => {
        if (elementsNodeList_header) {
            const burger = elementsNodeList_header.querySelector(".burger");
    ...
    function initMenus() {
        ...
        getElements(".menu-container").forEach((menuItem => {
            ...
            const menuContentsWrapper = getElement(".menu-contents", menuItem);
            initializeMenus();
            ...
            menuContents.forEach(((content, index) => {
                ...
                if (closeButton) closeButton.addEventListener("click", (() => closeMenu(index)));
            }));
            function handleMenuClick(index) {
                if (menuButtons[index].classList.contains("active")) closeMenu(index); else showMenu(index);
            }
            function closeMenu(index) {
                menuButtons[index].classList.remove("active");
                menuContents[index]?.classList.remove("active");
                if (menuContentsWrapper) menuContentsWrapper.classList.remove("active");
            }
            ...
            function showMenu(index) {
                ...
                if (menuContentsWrapper) menuContentsWrapper.classList.add("active");
            }
        }));
    }
    function initTabs() {
            ...
            on: {
                slideChange: function() {
                    ...
                    slides.forEach(((slide, index) => {
                        if (index === this.activeIndex) slide.classList.add("swiper-slide-hidden"); else slide.classList.remove("swiper-slide-hidden");
                    }));
                }
            }
        });
    ...
    function addClass(el, ...classes) {
        if (el.length === void 0) addClassEl(el, ...classes); else forEachHTML(el, (currEl => {
    ...
    class marquee {
        constructor(el, opts) {
            ...
            if (opts.speed) opts.duration = parseInt(el.clientWidth) / opts.speed * 1e3;
            ...
            this._opts = opts;
            this._calcSizes();
            const animationName = `marqueeAnimation-${Math.floor(Math.random() * 1e7)}`, animStr = this._animationStr(animationName, opts.duration / 1e3, opts.delayBeforeStart / 1e3, "infinite");
            this._animName = animationName;
            this._animStr = animStr;
            if (opts.duplicated) {
                if (vertical) if (opts.startVisible) this._marqWrap.style.transform = "translateY(0px)"; else this._marqWrap.style.transform = `translateY(${opts.direction === "up" ? this._contHeight : -1 * (this._elHeight * 2 - opts.gap)}px)`; else if (opts.startVisible) this._marqWrap.style.transform = "translateX(0px)"; else this._marqWrap.style.transform = `translateX(${opts.direction === "left" ? this._contWidth : -1 * (this._elWidth * 2 - opts.gap)}px)`;
                if (!opts.startVisible) this._loopCount = 1;
            } else if (opts.startVisible) this._loopCount = 2; else if (vertical) this._repositionVert(); else this._repositionHor();
        ...
        _animate(vertical = false) {
            ...
            if (opts.duplicated) {
                if (this._loopCount === 1) {
                    ...
                    if (vertical) duration = opts.direction === "up" ? duration + this._contHeight / (this._elHeight / duration) : duration * 2; else duration = opts.direction === "left" ? duration + this._contWidth / (this._elWidth / duration) : duration * 2;
                    this._animStr = this._animationStr(this._animName, duration / 1e3, opts.delayBeforeStart / 1e3);
                } else if (this._loopCount === 2) {
                    this._animName = `${this._animName}0`;
                    this._animStr = this._animationStr(this._animName, opts.duration / 1e3, 0, "infinite");
                }
                this._loopCount++;
            }
            let animationCss = "";
            if (vertical) if (opts.duplicated) {
                ...
                if (this._loopCount === 2) {
                    this._animStr = this._animationStr(this._animName, opts.duration / 1e3, opts.delayBeforeStart / 1e3);
            ...
            if (styles.length) styles[styles.length - 1].innerHTML = keyFrameCss; else if (byClass(`marq-wrap-style-${this._instance}`).length) byClass(`marq-wrap-style-${this._instance}`)[0].innerHTML = keyFrameCss; else {
        ...
        _calcSizes() {
            ...
            if (this._vertical) {
                ...
                forEachHTML(marqs, ((currEl, ind) => {
                    ...
                    if (opts.duplicated && ind === marqNums) currEl.style.marginBottom = "0px"; else currEl.style.marginBottom = `${opts.gap}px`;
                }));
                ...
                } else opts.duration = (elWidth + parseInt(contWidth)) / parseInt(contWidth) * this._duration;
            }
        ...
        toggle() {
            if (this._status === "paused") this.resume(); else if (this._status === "running") this.pause();
        }
        destroy() {
    ...
    function updateViewBox() {
        ...
        if (screenWidth >= 640) svgElement.setAttribute("viewBox", "0 0 18624.8 5000"); else svgElement.setAttribute("viewBox", "0 0 5000 5000");
    }
    updateViewBox();
